...
var RTLSDRDevice = /** @class */ (function () {
    ...
    RTLSDRDevice.prototype.getTunerGains = function (gains) {
        this.checkOpen();
        // @ts-ignore
        var gainCount = baremetal_1.librtlsdr.rtlsdr_get_tuner_gains(this.device, ref_napi_1.default.NULL);
        var gainsArray = Buffer.alloc(gainCount * ref_napi_1.default.types.int.size).fill(0);
        // @ts-ignore
        baremetal_1.librtlsdr.rtlsdr_get_tuner_gains(this.device, gainsArray);
        var arr = [];
        for (var i = 0; i < gainCount; i++) {
            arr.push(ref_napi_1.default.types.int.get(gainsArray, ref_napi_1.default.types.int.size * i));
        }
        return arr;
    };
    /**
