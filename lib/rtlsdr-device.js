...
var RTLSDRDevice = /** @class */ (function () {
    ...
    **/
    RTLSDRDevice.prototype.readAsync = function (callback, buf_num, buf_len) {
        ...
        // @ts-ignore
        var result = baremetal_1.librtlsdr.rtlsdr_read_async(this.device, rtlsdrCallback, ref_napi_1.default.NULL, buf_num, buf_len);
    };
    /**
    ...
    RTLSDRDevice.prototype.cancelAsync = function () {
        this.checkOpen();
        // @ts-ignore
        var result = baremetal_1.librtlsdr.rtlsdr_cancel_async(this.device);
        if (result !== 0)
            console.error("cancel async error " + typeof (result) + " " + result);
    };
    return RTLSDRDevice;
}());
